...
from src.data.database import StockData, OptionsData
...
def display_database_explorer():
    ...
    st.caption("Navigate the explorer with the subtabs below to keep related actions grouped together.")

    data_handler = st.session_state.data_handler

    # Precompute shared values used across subtabs
    ...
    overview_tab, stock_tab, options_tab, query_tab = st.tabs([
        "Overview",
        "Stock Data",
        "Options Data",
        "Query & Visualization"
...
{str(e)}
    ...
    with options_tab:
        st.subheader("Download Options Data")
...
{str(e)}
    ...
    with query_tab:
        ...
        start_date_default = min_date if min_date else datetime.now() - timedelta(days=365)
        end_date_default = max_date if max_date else datetime.now()

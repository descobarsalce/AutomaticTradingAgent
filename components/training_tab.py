...
def display_training_tab():
    ...
    st.session_state.stock_name = st.text_input(
        "Training Stock Symbol", value=st.session_state.stock_name)
    ...
    with train_col1:
        st.session_state.train_start_date = datetime.combine(
            st.date_input("Training Start Date",
                          value=datetime.now() - timedelta(days=365 * 5)),
            datetime.min.time())
    with train_col2:
        st.session_state.train_end_date = datetime.combine(
    ...
    with tab1:
        st.session_state.ppo_params = display_manual_parameters()
    ...
    with test_col1:
        st.session_state.test_start_date = datetime.combine(
            st.date_input("Test Start Date",
                          value=datetime.now() - timedelta(days=365)),
            datetime.min.time())
    with test_col2:
        st.session_state.test_end_date = datetime.combine(
...
def display_manual_parameters() -> Dict[str, Any]:
    ...
    use_optuna_params = st.checkbox(
        "Use Optuna (previously) Optimized Parameters", value=False)

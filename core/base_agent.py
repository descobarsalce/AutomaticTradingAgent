...
class UnifiedTradingAgent:
    ...
    @type_check
    def prepare_processed_data(self, stock_name: str, start_date: datetime, end_date: datetime) -> pd.DataFrame:
        """Fetch and prepare data."""
        portfolio_data = self.data_handler.fetch_data([stock_name], start_date, end_date)
    ...
    @type_check
    def initialize_env(self, data: pd.DataFrame, env_params: Dict[str, Any]) -> None:
    ...
    def configure_ppo(self, ppo_params: Optional[Dict[str, Any]] = None) -> None:
        ...
        for param, value in self.ppo_params.items():
            if param in self.PARAM_RANGES and value is not None:
                ...
                if not (min_val <= value <= max_val):
                    logger.warning(f"Parameter {param} value {value} outside range [{min_val}, {max_val}]")
        ...
        policy_kwargs = (
            ...
            else self.DEFAULT_POLICY_KWARGS.copy()
        )
    ...
    def update_state(self, portfolio_value: float, positions: Dict[str, float]) -> None:
        ...
        try:
            if self.env and hasattr(self.env, 'data'):
                current_prices = {symbol: self.env.data.loc[self.env.current_step, 'Close'] 
                ...
                if not np.isclose(total_calc, portfolio_value, rtol=1e-3):
                    logger.warning(f"Portfolio value mismatch: calculated={total_calc:.2f}, reported={portfolio_value:.2f}")
        except Exception as e:

...
class HyperparameterOptimizer:
    ...
    def __init__(self,
        ...
        if param_grid is None:
            if fast_mode:
                ...
                self.param_grid = {
                    'learning_rate':
                    [1e-4, 3e-4],  # Focused around common optimal values
    ...
    def evaluate_params(self,
                        agent: TradingAgent,
                        n_episodes: int = 5,
                        early_stop: bool = True) -> Tuple[float, bool]:
        """
            ...
            if early_stop and episode >= min_episodes:
                ...
                if reward_threshold and current_avg < reward_threshold:
                    ...
                    if consecutive_poor_episodes >= 2:  # Two consecutive poor episodes
                        logger.info(
                            f"Stopping early: Performance below threshold for {consecutive_poor_episodes} episodes"
                        )
                ...
                if reward_std / (abs(current_avg) +
                                 1e-8) < 0.05 and episode >= min_episodes * 2:
    ...
    def optimize(
        ...
        for idx, params in enumerate(param_combinations, 1):
            ...
            if status_placeholder is not None:
                ...
                estimated_total_time = elapsed_time * (total_combinations /
                                                       idx)
            ...
            try:
                ...
                if fast_mode:
                    ...
                    train_steps = min(total_timesteps // 16,
                                      250)  # Minimal training
                    ...
                    if self.best_reward > float(
                            '-inf') and reward < self.best_reward * 0.4:
                        continue
                    ...
                    avg_reward, should_stop = self.evaluate_params(
                        agent, n_episodes=n_eval_episodes, early_stop=True)
                ...
                if avg_reward > self.best_reward:
                    ...
                    if improvement > self.min_improvement:
                        ...
                        trials_without_improvement = 0
                        logger.info(
                            f"New best parameters found! Average reward: {avg_reward}"
                        )
                ...
                if trials_without_improvement >= self.patience:
                    logger.info(
                        "Early stopping: No significant improvement for several trials"
                    )
                    break

                # Early stopping if we reached a good enough reward
                theoretical_max = self.env.reward_range[1] if hasattr(
                    self.env, 'reward_range') else float('inf')
    ...
    def get_optimization_summary(self) -> Dict[str, Any]:
        ...
        if not self.results:
            return {
                ...
                "message":
                "No optimization results available. The process may have failed or been interrupted.",
                "best_params":
                self.
                best_params,  # Return default params if optimization failed
                "best_reward":
                self.best_reward if self.best_reward != float('-inf') else 0.0,
        ...
        return {
            ...
            "message":
            f"Successfully tested {len(self.results)} parameter combinations",

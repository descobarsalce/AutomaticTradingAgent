            ...
            def plot_discrete_actions(info_history: List[Dict], fig: Optional[go.Figure] = None) -> go.Figure:
            ...
            def plot_actions_with_price(info_history: List[Dict], price_data: pd.DataFrame) -> go.Figure:
            ...
            # =========================================================================
            #  NEW & IMPROVED FINANCE VISUALIZATIONS
            # =========================================================================

            def plot_correlation_heatmap(portfolio_data: Dict[str, pd.DataFrame], price_col: str = 'Close') -> go.Figure:
                """
            ...
            def plot_cumulative_returns(portfolio_data: Dict[str, pd.DataFrame], price_col: str = 'Close') -> go.Figure:
                """
            ...
            def plot_drawdown(portfolio_data: Dict[str, pd.DataFrame], symbol: str, price_col: str = 'Close') -> go.Figure:
                """
                ...
                if df.empty or price_col not in df.columns:
                    raise ValueError(f"No valid data for symbol '{symbol}' to plot drawdown.")
                ...
                fig.update_layout(
                    title=f"{symbol} Cumulative Returns & Drawdown",
                    template='plotly_dark',
                    hovermode='x unified'
                )
            ...
            def plot_performance_and_drawdown(portfolio_data: Dict[str, pd.DataFrame], symbol: str, price_col: str = 'Close') -> go.Figure:
                """
                ...
                fig = make_subplots(rows=2, cols=1, shared_xaxes=True,
                                    vertical_spacing=0.03,
                                    subplot_titles=[f"{symbol} Cumulative Returns", f"{symbol} Drawdown"])
                ...
                fig.update_layout(
                    title=f"{symbol} Performance & Drawdown",
                    template='plotly_dark',
                    hovermode='x unified',
                    height=800
                )

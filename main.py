...
def main():
    ...
    with col1:
        ...
        transaction_cost = st.number_input("Transaction Cost",
                                           value=0.0,
                                           step=0.001)
        min_transaction_size = st.number_input("Minimum Transaction Size",
                                               value=0.001,
                                               step=0.001)
    with col2:
        max_position_pct = st.number_input("Maximum Position %",
                                           value=0.95,
                                           min_value=0.0,
                                           max_value=1.0)
    ...
    with col3:
        learning_rate = st.number_input(
            ...
            value=3e-4,
            format="%.1e",
            help="Suggested range: 1e-5 to 1e-3. Default: 3e-4")
        n_steps = st.number_input(
            "Number of Steps",
            value=512,
            help="Suggested range: 64 to 2048. Default: 256")
    ...
    with col4:
        gamma = st.number_input(
            ...
            value=0.99,
            help="Suggested range: 0.8 to 0.999. Default: 0.99")
        clip_range = st.number_input(
            "Clip Range",
            value=0.2,
            help="Suggested range: 0.0 to 0.5. Default: 0.2")
        target_kl = st.number_input(
            "Target KL Divergence",
            value=0.05,
            help="Suggested range: 0.01 to 0.1. Default: 0.05")
    ...
    if col_train.button("Start Training"):
        ...
        if not portfolio_data:
            st.error(
                "No data found in database. Please add some stock data first.")
            return
            ...
            with st.expander("Test Results", expanded=True):
                ...
                while not done:
                    ...
                    metrics_placeholder.write({
                        ...
                        'Portfolio Value':
                        round(info['net_worth'], 2),
                        'Position':
                        round(float(test_env.shares_held), 3)
                    })
                ...
                st.success(
                    f"Test completed! Final portfolio value: ${info['net_worth']:.2f}"
                )

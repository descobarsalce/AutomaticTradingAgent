...
def calculate_returns(portfolio_history: List[float]) -> np.ndarray:
    ...
    try:
        ...
        if len(returns) > 0:
            ...
            logger.debug(
                f"Calculated returns with {len(returns)} valid data points")
...
def calculate_volatility(returns: np.ndarray, annualize: bool = True) -> float:
    ...
    try:
        ...
        if len(valid_returns) <= 1:
            logger.debug(
                "Insufficient valid return values for volatility calculation")
            return 0.0
...
def calculate_sharpe_ratio(returns: np.ndarray) -> float:
    ...
    if len(returns
           ) <= 252:  # Minimum one year of data for meaningful Sharpe ratio
        logger.debug(
    ...
    try:
        ...
        if len(valid_returns) <= 1:
            logger.debug(
                "Insufficient valid return values for Sharpe ratio calculation"
            )
            return 0.0
...
def calculate_sortino_ratio(returns: np.ndarray) -> float:
    ...
    try:
        ...
        if len(valid_returns) <= 1:
            logger.debug(
                "Insufficient valid return values for Sortino ratio calculation"
            )
            return 0.0
        ...
        if len(negative_returns) == 0:
            logger.debug(
                "No negative returns found for Sortino ratio calculation")
        ...
        if downside_std > 1e-8:  # Avoid division by very small numbers
            ...
            sortino_clipped = float(np.clip(sortino, -100,
                                            100))  # Limit extreme values
            logger.debug(f"Calculated Sortino ratio: {sortino_clipped}")
            return sortino_clipped
        else:
            logger.warning(
                "Downside deviation too small for reliable Sortino ratio")
            return 0.0
...
def calculate_information_ratio(
    ...
    try:
        ...
        if len(valid_returns) <= 1:
            logger.debug(
                "Insufficient valid return values for Information ratio calculation"
            )
            return 0.0
        ...
        if not np.isfinite(avg_excess_return) or not np.isfinite(
                tracking_error):
            logger.warning(
                "Non-finite values in Information ratio calculation")
            return 0.0
        ...
        if tracking_error > 1e-8:  # Avoid division by very small numbers
            ...
            information_ratio_clipped = float(
                np.clip(information_ratio, -100, 100))
            logger.debug(
                f"Calculated Information ratio: {information_ratio_clipped}")
            return information_ratio_clipped
        else:
            logger.warning(
                "Tracking error too small for reliable Information ratio")
            return 0.0
...
def calculate_beta(returns: np.ndarray, market_returns: np.ndarray) -> float:
    ...
    try:
        ...
        if market_variance > 1e-8:  # Avoid division by very small numbers
            ...
            beta_clipped = float(np.clip(beta, -10,
                                         10))  # Limit extreme values

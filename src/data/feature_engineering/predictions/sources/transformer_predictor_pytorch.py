...
from src.data.feature_engineering.predictions.base_prediction import (
        ...
        try:
            ...
            if os.path.exists(config_path):
                with open(config_path, 'r') as f:
                    ...
                    self._d_model = config.get('d_model', 64)
                    self._num_heads = config.get('num_heads', 4)
                    self._num_layers = config.get('num_layers', 2)
                    self._ff_dim = config.get('ff_dim', 128)
                    self._dropout = config.get('dropout', 0.1)
